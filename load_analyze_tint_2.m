...
%% load and analyze the tint 
    ...
    j_ok = mean(mmsd.divovercurl.data, 'omitnan') < 0.3;
    % Test if j error is low enough to use j data
    % p--------->possibly at a later point exclude all follow up calculations and just
    % set the values in value table to NAN for event with divovercurl>0.3!
    if mean(mmsd.divovercurl.data, 'omitnan') < 0.3
        fprintf('Error on j small enough.: Q = %.2f', mean(mmsd.divovercurl.data, 'omitnan'))
    else
    ...
    event.J_pmm = event.raw.Ni1*1.609*10^(-19)*(event.raw.Vi1_gsm - event.raw.Ve1_gsm);
    %% J*r stuff
    ...
    %% analyzing
    position = [mean(event.raw.R1_gsm.data(:,1)), mean(event.raw.R1_gsm.data(:,2)), mean(event.raw.R1_gsm.data(:,3))]; 
    % J*r direction
