function [omni, mmsd, event, data, current_info, errors_for_table, error, error_position] = load_analyze_tint_2(tint, tint_beob, Param, i)
...
%% load and analyze the tint 
    ...
    % calculating error on j
    error.B_gsm = ones(length(event.raw.B1_gsm),1); error.B_gsm = TSeries(event.raw.B1_gsm.time,error.B_gsm*0.1*10^(-9));
    error.Jx = sqrt(((error.B_gsm.data(:,1))./(event.raw.Rres2.data(:,1)-event.raw.Rres1.data(:,1))).^2+((error.B_gsm.data)./(event.raw.Rres3.data(:,1)-event.raw.Rres2.data(:,1))).^2+((error.B_gsm.data)./(event.raw.Rres1.data(:,1)-event.raw.Rres3.data(:,1))).^2);
    error.Jy = sqrt(((error.B_gsm.data(:,1))./(event.raw.Rres2.data(:,2)-event.raw.Rres1.data(:,2))).^2+((error.B_gsm.data)./(event.raw.Rres3.data(:,2)-event.raw.Rres2.data(:,2))).^2+((error.B_gsm.data)./(event.raw.Rres1.data(:,2)-event.raw.Rres3.data(:,2))).^2);
    error.Jz = sqrt(((error.B_gsm.data(:,1))./(event.raw.Rres2.data(:,3)-event.raw.Rres1.data(:,3))).^2+((error.B_gsm.data)./(event.raw.Rres3.data(:,3)-event.raw.Rres2.data(:,3))).^2+((error.B_gsm.data)./(event.raw.Rres1.data(:,3)-event.raw.Rres3.data(:,3))).^2);
    
    Jx=event.raw.f.j.data(:,1);
    Jy=event.raw.f.j.data(:,2);
    Jz=event.raw.f.j.data(:,3);
    error.J = sqrt(((Jx./sqrt(Jx.^2+Jy.^2+Jz.^2)).*error.Jx).^2 + ((Jy./sqrt(Jx.^2+Jy.^2+Jz.^2)).*error.Jy).^2 + ((Jy./sqrt(Jx.^2+Jy.^2+Jz.^2)).*error.Jy).^2);
    ...
    error.Jused = TSeries(event.raw.j.time,error.J(tmp.ind,:));
    error.Jvec_used = [error.Jx(tmp.ind,:), error.Jy(tmp.ind,:), error.Jz(tmp.ind,:)];
%% filter out "noise"
    ...
    nslv = event.medj + 2*event.stdj; % 2*event.stdj; 
    ...
    % j_mag
    % test if errJ and J same size
    test = length(error.Jused.data) == length(event.raw.j_mag.data);
    if test == 0
        fprintf('Error of J not same size as J')
    end
    tmp.D_pks = zeros(size(event.raw.j_mag.data));
    tmp.D_pks(:) = nan; tmp.D_pks(event.locsj) = event.pksj; 
    %tmp.errJpks = zeros(size(error.Jused.data)); tmp.errJpks(:) = nan; 
    %tmp.errJpks(event.locsj) = error.Jused.data(event.locsj);

    event.Jmag_pks = TSeries(event.raw.j_mag.time, tmp.D_pks); %j magnitude peaks
    error.Jmag_pks = error.Jused.data(event.locsj);
    event.j_pks = TSeries(event.raw.j.time(event.locsj), event.raw.j.data(event.locsj,:)); %j peaks
    error.J_pks = error.Jvec_used(event.locsj,:);
    % noise level definition from salinas but changed:
    event.nslv = TSeries(event.raw.j_mag.time, nslv*ones(length(event.raw.j_mag.time), 1)); % noise level definition from salinas
    ...
    %calculating error
    if length(error.Jvec_used) ~= length(event.raw.j.data)
        fprintf('Error of J not same size as J \n')
    end
    error.Juv = sqrt(((error.Jvec_used./vecnorm(event.raw.j.data, 2, 2)).^2) + (event.raw.j.data ./(event.raw.j_mag.data.^2).*error.Jused.data).^2);
    error.Juv_pks = error.Juv(event.locsj,:);
    ...
    % calculate error
    error.jr_uv = sum(error.Juv .* event.uv.r.data, 2);
    error.jr_uv_pks = sum(error.Juv_pks .* event.uv.r.data(event.locsj), 2);
    error.jr_mag = sum(error.Jused.data .* event.uv.r.data, 2);
    error.jr_mag_pks = sum(error.Jmag_pks .* event.uv.r.data(event.locsj), 2);
    % analyzing
    position = [mean(event.raw.R1_gsm.data(:,1)./6378), mean(event.raw.R1_gsm.data(:,2)./6378), mean(event.raw.R1_gsm.data(:,3)./6378)]; 
    error_position = [std(event.raw.R1_gsm.data(:,1)./6378),std(event.raw.R1_gsm.data(:,2)./6378),std(event.raw.R1_gsm.data(:,3)./6378)];
    % J*r direction
    ...
    tot_curr_a25_pks = sum(event.jr_mag.pks.data(find(event.uv.jr_pks.data > 0.25)))+sum(event.jr_mag.pks.data(find(event.uv.jr_pks.data < -0.25)));
        % error calculation        
        % for mag total value of current
    error.tot_curr_all = abs(sum(error.jr_mag(find(event.uv.jr_all.data > 0)))+sum(error.jr_mag(find(event.uv.jr_all.data < 0))));
    error.tot_curr_pks = abs(sum(error.jr_mag_pks(find(event.uv.jr_pks.data > 0)))+sum(error.jr_mag_pks(find(event.uv.jr_pks.data < 0))));
    error.tot_curr_a25_pks = abs(sum(error.jr_mag_pks(find(event.uv.jr_all.data(event.locsj) > 0.25)))+ sum(error.jr_mag_pks(find(event.uv.jr_all.data(event.locsj) < -0.25))));
    ...
    errors_for_table = [error.tot_curr_all(1,1)*1e6, error.tot_curr_pks(1,1)*1e9, error.tot_curr_a25_pks(1,1)*1e9];
    
    tmp.pks = zeros(length(event.raw.j_mag.data),3);
    tmp.pks = nan;
    tmp.pks = [tmp.pks,tmp.pks,tmp.pks];
    tmp.pks(event.locsj,:) = error.Juv_pks; 
    error_Juv_pks_rightsize = tmp.pks(:);

    size(error.J(tmp.ind,:))
    size(error.Juv(:,:))
    size(error_Juv_pks_rightsize(:,:)) % this one is wrong

    %errors_for_plots = [error.J(tmp.ind,:), error.Juv(:,:), error_Juv_pks_rightsize(:,:)];
end