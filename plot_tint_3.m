...
%% Current peaks analysing for SINGLE event
...
tmp.filename = fullfile('events/plots/','interesting/');
if ~exist(tmp.filename, 'dir')
    mkdir(tmp.filename);
end
tmp.filename = fullfile('events/plots/','interesting/', event.tint_string);
irf_print_fig(tmp.filename,'png');
%close(gcf);
